...
features:
  ...
  - "組織ポリシーによるプラグインブロック"
  - "OTELツールパラメータのゲーティング"
codex_review: "今回の本丸は派手な新機能より、hooks条件分岐やOAuth整備のような運用の摩擦を減らす地味筋だと思う。一般受けは弱いが、エンタープライズ導入の現実味を一段押し上げる更新としてはかなり重要です。 ※ このレビューは後日生成されました"
codex_importance: 4
---
...
> hooksにif条件フィルタが追加。permission rule構文でhook実行条件を指定可能に。minutesを節約、process spawnsが12→3に減りfree perf win。 — @jarredsumner [X](https://x.com/i/status/2037675111172743184)

> Permission rule syntax is a game-changer. hooksの柔軟性が大幅向上。特定コマンドのみhook発火が可能に。 — @lydiahallie [X](https://x.com/i/status/2037641086391533839)

> repos that use hooks meant only for one commandに最適。複数のstop hooksを1つに統合できる。 — @niko [X](https://x.com/i/status/2037476100922597871)

> 不要なプロセス起動を削減、CIも軽くなりそう。hooksガッツリ使ってる人にはかなり嬉しい。地味アプデだが積み重なって使い勝手変わる。 — @hiro8ma [X](https://x.com/i/status/2037818222112833852)

> ワークフロー自動化の精度を一段引き上げる。`Bash(git *)`でgitコマンド時のみ発火。 — @oikon48 [X](https://x.com/i/status/2037664421154378088)

> 結構使えそうな予感。Claudeに確認して試した。 — @cc_lab_jp [X](https://x.com/i/status/2037510836680302804)
